    ...
    try:
        for i in range(data7.shape[0]):
            ...
            while counter <= 15:
                ...
                # lower wick
                u2 = abs(data7.iloc[i-counter]['low'] - data7.iloc[i-counter]['open'])
                ...
                # lower wick
                d2 = abs(data7.iloc[i-counter]['close'] - data7.iloc[i-counter]['low'])
                if (data7.iloc[i-counter]['low'] < data7.iloc[i-(counter-1)]['low'] and data7.iloc[i-counter]['low'] < data7.iloc[i-(counter+1)]['low'] and data7.iloc[i-counter][u] < data7.iloc[i-counter][u2] and data7.iloc[i-counter][u] > data7.iloc[i-counter][u1]):
                    return 'bullish pin'
                    #pb.append(data7.iloc[i-counter]['time'])
                elif (data7.iloc[i-counter]['high'] > data7.iloc[i-(counter-1)]['high'] and data7.iloc[i-counter]['high'] > data7.iloc[i-(counter+1)]['high'] and data7.iloc[i-counter][d] > data7.iloc[i-counter][d2] and data7.iloc[i-counter][d] < data7.iloc[i-counter][d1]):
                    return 'bearish pin'
    ...
    try:
        for i in range(data7.shape[0]):
            ...
            while counter <= 15:
                ...
                # bearish body
                d = abs(data7.iloc[i-counter]['open'] - data7.iloc[i-counter]['close'])
                if (data7.iloc[i-(counter - 1)]['candle_type'] == ('bearish') and data7.iloc[i-counter]['candle_type'] == ('bullish') and data7.iloc[i-counter][u] > data7.iloc[i-(counter-1)][d]):
                    #return 'bullish engulf'
                    eb.append(data7.iloc[i-counter]['time'])
                elif(data7.iloc[i-(counter - 1)]['candle_type'] == ('bullish') and data7.iloc[i-counter]['candle_type'] == ('bearish') and data7.iloc[i-counter][d] > data7.iloc[i-(counter-1)][u]):
                    #return 'bearish engulf'
                    el.append(data7.iloc[i-counter]['time'])
                else:
                    pass
    except IndexError:
        pass
    engulf =  pd.DataFrame(e, columns=['pinbar'])
    etm3 = pd.DataFrame(eb, columns = ['buy pinbar'])
    etm4 = pd.DataFrame(el, columns = ['sell pinbar'])
    etm3.to_csv('ETM7.csv')
    etm4.to_csv('ETM727.csv')
    engulf.to_csv('engulf7.csv')
    return engulf,etm3,etm4
